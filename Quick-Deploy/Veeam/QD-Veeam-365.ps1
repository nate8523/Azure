<#	
	...
	===========================================================================
     ...
     Version:       2.0
	 Filename:     	QD-Veeam-365.ps1
	===========================================================================
	
    .DESCRIPTION
		This script is used to provision a new Veeam 365 Backup Solution into an Azure tenant.
        The Solution Configures a new Resource Group containing a VNET, Storage Account,
...
Start-Transcript -Path "$LogPath\QD-Veeam-365.log" -Append
...
$ResourceGroup = New-AzResourceGroup -Name "$CustomerPrefix-RG-V365-01" -Location $Location

# Create Storage Account
Write-Host "Deploying Storage Account for VM Diagnostics"
$StorageAccount = New-AzStorageAccount -ResourceGroupName $ResourceGroup.ResourceGroupName -AccountName ("$CustomerPrefix" + "v365diagstore011").ToLower() -Location $Location -Type "Standard_LRS" -Kind "Storagev2"
...
$VeeamPiP = New-AzPublicIpAddress -ResourceGroupName $ResourceGroup.ResourceGroupName -Location $Location -Name "$CustomerPrefix-PiP-V365-01" -AllocationMethod Static -IdleTimeoutInMinutes 4
...
$VeeamNSG = New-AzNetworkSecurityGroup -ResourceGroupName $ResourceGroup.ResourceGroupName -Location $Location -Name "$CustomerPrefix-NSG-V365-01" -SecurityRules $nsgAllowRDP,$nsgCCrule1,$nsgCCrule2

# Create a virtual network card and associate with public IP address and NSG
Write-Host "Deploying Network Interface for Veeam VM"
$VeeamNIC = New-AzNetworkInterface -Name "$CustomerPrefix-V365-NIC-01" -ResourceGroupName $ResourceGroup.ResourceGroupName -Location $Location -SubnetId $VeeamNetwork.Subnets[0].Id -PublicIpAddressId $VeeamPiP.Id -NetworkSecurityGroupId $VeeamNSG.Id

# Set the Marketplace image
$Publisher = "veeam"
$offerName = "office365backup"
$skuName = "veeamoffice365backup"
$version = "5.1.252"
...
$vmConfig = New-AzVMConfig -VMName "$CustomerPrefix-V365-01" -VMSize "Standard_B1ms"
$vmConfig = Set-AzVMPlan -VM $vmConfig -Publisher $Publisher -Product $offerName -Name $skuName
$vmConfig = Set-AzVMOperatingSystem -Windows -VM $vmConfig -ComputerName "$CustomerPrefix-V365-01" -Credential $VMCredentials
...
$vmConfig = Set-AzVMOSDisk -VM $vmConfig -CreateOption "FromImage" -Name "$CustomerPrefix-V365-01-OSDisk"
...
$vmDataDisk01 = New-AzDisk -DiskName "$CustomerPrefix-V365-01-DataDisk-1" -Disk $vmDataDisk01Config -ResourceGroupName $ResourceGroup.ResourceGroupName
$vmConfig = Add-AzVMDataDisk -VM $vmConfig -Name "$CustomerPrefix-V365-01-DataDisk-1" -CreateOption Attach -ManagedDiskId $vmDataDisk01.Id -Lun 0
